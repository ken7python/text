...
def click_save_button(event):  # 保存ボタンをクリック時の動作
    if not hifa == "":
        data = codecs.open(hifa, "w" , 'shift_jis')
...
def faop(ofa):
    ...
    global hifa
    hifa = ofa
    data = codecs.open(hifa, "rb",'shift_jis')
    # すべての内容を読み込む
    contents = data.read()
...
def fileopen(event):
    ...
    faop(fl)
def komando(k):
    result = subprocess.run([k,str(os.path.split(hifa)[1])], stdout=subprocess.PIPE,encoding='shift_jis')
    global kekka
    kekka = result.stdout
    koma.SetValue(kekka)
def java(event):
